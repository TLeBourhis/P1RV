...
void Boss::fight(list<Character*> allies, list<Character*> ennemies){
  ...
  if(size(ennemies) == 0){
    ...
    if(distMin > attackRange){
      ...
      //Attaque de l'ennemi
      if(rage >= 100){ //ATTENTION : Mettre ce paramètre dans Param
        rage = 0;
        this->boostedAttack(nearestEnnemy);
      }else{
        nearestEnnemy->getHit(attackDamage,"ATTACK_DAMAGE");
        rage += 20; //ATTENTION : Mettre ce paramètre d'augmentation de rage dans Param
      }

      if(mana == 100){ //ATTENTION : Mettre ce paramètre dans Param
        mana = 0;
        this->spell1(ennemies);
      }else{
        mana += 10; //ATTENTION : Mettre ce paramètre dans Param
      }
    }
  }
}
